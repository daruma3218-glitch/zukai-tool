...
- excerpt に出てこない国・地名は入れない。国境は現代のもので、日本政府の立場で描く
  （北方領土・竹島・尖閣は日本、クリミアはウクライナ）。
- 南樺太（北緯50度以南）と千島列島（得撫島〜占守島）は帰属未定のため、どの国の色も塗られない
  （RUS を強調してもここは塗られない）。その地域そのものを示す時だけ highlight に入れる：
  南樺太は "XSS"、千島列島は "XKR"（別々に指定できる。日露戦争で割譲されたのは南樺太だけ）。
- 国名が抜粋に無く、海・海峡・半島・航路だけが出てくる時（例: 黒海から地中海へ）は、focus と highlight を
  空にし、pins にその海・海峡の名前（抜粋にある語）と代表地点の座標を入れ、arrows で pins を結ぶ。
- 強調する国と同じ名前のピン（例: ロシア）は作らない。国から出る矢印は、from / to に国コードを書く。
"""
